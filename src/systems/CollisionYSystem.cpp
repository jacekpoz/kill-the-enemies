...
#include "CollisionYSystem.hpp"

#include "../math.hpp"

CollisionYSystem::CollisionYSystem(entt::registry &r, Config &c) 
	: System(r, c) {}

void CollisionYSystem::update(const float deltaTime) {
	if (paused) return;
	...
	for (const auto m : movables) {
 		...
 		for (const auto mm : movables) {
 			...
 			if (collides(mHitbox, mPos, mmHitbox, mmPos)) 
 				resolveCollisionY(m, mm, deltaTime);
 		}
 		for (const auto i : immovables) {
 			const auto [iHitbox, iPos] = registry.get<Hitbox, Position>(i);
 			if (collides(mHitbox, mPos, iHitbox, iPos)) 
 				resolveCollisionY(m, i, deltaTime);
		}
	}
}

void CollisionYSystem::resolveCollisionY(const entt::entity e1, const entt::entity e2, const float deltaTime) {
	if (e1 == e2) return;
	...
	if (registry.all_of<Movable>(e1) && registry.all_of<Movable>(e2)) {
		...
		registry.patch<Velocity>(e1, [deltaTime, e1Pos, e2Pos](auto &vel) {
			vel.y -= (e2Pos.y - e1Pos.y) * deltaTime * 25.f;
		});
	...
	if (registry.all_of<Movable>(e1) && registry.all_of<Immovable>(e2)) 
		movableImmovableY(e1, e2, deltaTime);
	if (registry.all_of<Immovable>(e1) && registry.all_of<Movable>(e2)) 
		movableImmovableY(e2, e1, deltaTime);
}

void CollisionYSystem::movableImmovableY(const entt::entity m, const entt::entity i, const float deltaTime) {
	if (m == i) return;

 	if (!registry.all_of<Movable>(m)) return;
 	if (!registry.all_of<Immovable>(i)) return;

	registry.patch<Position>(m, [](auto &p) { p.y = p.lastY; });
	registry.patch<Velocity>(m, [](auto &v) { v.y = 0.f; });
	registry.patch<Acceleration>(m, [](auto &a) { a.y = 0.f; });
}

void CollisionYSystem::projectileCollision(const entt::entity proj, const entt::entity e, const float deltaTime) {
	if (proj == e) return;
...
void CollisionYSystem::playerEnemyCollision(const entt::entity pl, const entt::entity e, const float deltaTime) {
